...
function initializeStudentAssignments(){ const logout = document.getElementById('student-logout-button'); const studentSel = document.getElementById('student-select'); const typeSel = document.getElementById('student-filter-type'); const classSel = document.getElementById('student-filter-class'); const search = document.getElementById('student-assign-search');
  buildStudentOptions(); studentSel?.addEventListener('change', ()=> renderForCurrentSelection()); typeSel?.addEventListener('change', ()=> renderForCurrentSelection()); classSel?.addEventListener('change', ()=> renderForCurrentSelection()); search?.addEventListener('input', ()=> renderForCurrentSelection()); logout?.addEventListener('click', async ()=>{ if(typeof logoutStudent === 'function'){ await logoutStudent(); } else { redirectToLogin(); } }); renderForCurrentSelection(); bindStudentActions(studentSel?.value || ''); }

window.addEventListener('DOMContentLoaded', async ()=>{ if(typeof isStudentAuthenticated === 'function' && !(await isStudentAuthenticated())){ redirectToLogin(); return; } initializeStudentAssignments(); });
